...
function buildSignalList(role: string): string[] {
  switch (role) {
    case "user":
      return ["process.message", "process.context", "chat.text", "chat.tool_call", "chat.tool_result", "chat.hil", "chat.complete", "process.exit", "device.status", "adapter.status", "pkg.changed"];
    case "driver":
